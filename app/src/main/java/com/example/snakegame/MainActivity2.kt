...
class MainActivity2 : Activity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val handler = Handler()
        var delayMillis = 150L // Update snake position every 100 milliseconds
        ...
        newgame.setOnClickListener {
            ...
            val randomX =
                random.nextInt(301) - 300 // generate a random x-coordinate between -400 and 400
            val randomY =
                random.nextInt(301) - 300 // generate a random y-coordinate between -400 and 400
            ...
            val runnable = object : Runnable {
                override fun run() {
                    ...
                    when (currentDirection) {
                        "up" -> {
                            snakeY -= 10
                            if (snakeY < -350) { // Check if the ImageView goes off the top of the board
                                snakeY = -350f
                        ...
                        "left" -> {
                            snakeX -= 10
                            if (snakeX < -350) { // Check if the ImageView goes off the top of the board
                                snakeX = -350f
                                border.setBackgroundColor(getResources().getColor(R.color.red))
                                playagain.visibility = View.VISIBLE
                                currentDirection = "pause"
